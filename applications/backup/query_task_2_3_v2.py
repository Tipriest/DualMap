...
- 解析用户指令提取 target_room, target_object, related_object
...
class Task23Subscriber(TaskSubscriber):
    ...
    def _task_worker_v2(self):
        """
        Task 2-3 的工作流程：找到目标物体（可能带相关物体定位）

        执行流程：
        1. 等待房间信息就绪（如果指定了房间）
        2. 在 CLIP map 中查询 target 的初步位置（不一定准确）
        ...
        关键点：
        - query_callback 只是粗略定位，精确位置由 dualmap 实时检索提供
        - 旋转过程中通过 remap_target_callback 接收 dualmap 的结果
        - 必须先导航到目标位置，再执行 check_task (RGB 验证)
        """
        while not self._shutdown_event.is_set():
            ...
            try:
                # ===== 步骤1: 等待房间信息就绪 =====
                # 如果用户指定了房间（如"去卧室找杯子"），需要等待房间边界信息准备好
                # room_bbox 用于过滤 CLIP map 中的物体，只在指定房间内搜索
                ...
                # 注意：这里的 free_x, free_y 暂时不用，因为会等 dualmap 的精确位置
                ...
                # ===== 步骤3: 导航到观察位置 =====
                # 策略：优先去 related 物体位置，如果没有则去房间锚点
                # 原因：related 物体（如"床"）通常离 target（如"被子"）更近
                nav_ok = False
                rx, ry, delta_rx, delta_ry = None, None, None, None

                if related_name != "None":
                    # 有相关物体，尝试在 CLIP map 中找到它
                    rcorners = self.query_callback(related_name)
                    if rcorners is None:
                        # 找不到相关物体，fallback 到房间锚点
                        ...
                        if self.room_anchor_pt is not None:
                            ...
                            self.get_logger().info(f"→ 前往房间锚点: ({anchor_x:.3f}, {anchor_y:.3f})")
                        ...
                        # delta_rx, delta_ry 是 bbox 的宽高，用于发布给 dualmap
                    ...
                    if self.room_anchor_pt is not None:
                        anchor_x, anchor_y = self.room_anchor_pt
                        self.get_logger().info(f"→ 无相关物体，前往房间锚点: ({anchor_x:.3f}, {anchor_y:.3f})")
                        nav_ok = self._goto_point(anchor_x, anchor_y, yaw=0.0, frame_id="map", wait_timeout=5.0)
                ...
                # 原理：dualmap 会订阅我们发布的 bbox 信息，在该区域实时检索目标
                # 我们边旋转边等待，一旦 dualmap 找到目标会通过 remap_target_callback 更新 target_x/target_y
                self.get_logger().info(f"===== 开始旋转 {self.max_spin_rounds} 圈等待 dualmap 返回结果 =====")

                # 发布检索请求给 dualmap
                # 两种模式：
                # 1. 如果有 related 物体，以 related 物体为中心检索（范围较小，更精确）
                # 2. 如果没有，以整个房间为范围检索（范围较大）
                if related_name != "None" and rx is not None:
                    self.get_logger().info(f"📤 发布检索请求: 在 '{related_name}' 附近搜索 '{target_name}'")
                    # 发布相关物体的中心和尺寸给 dualmap
                ...
                # 开始旋转并等待 dualmap 返回结果
                # 返回 True 表示找到了目标（target_x/target_y 被更新）
