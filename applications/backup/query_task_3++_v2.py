...
class Task3PPSubscriber(TaskSubscriber):
    ...
    def _handle_target_found(self):
        ...
        流程说明:
        1. 读取 dualmap 返回的目标位置 (target_x, target_y)
        2. 计算最近的可达点（避开障碍物）
        3. 导航到可达点并面向目标
        4. 使用 RGB 相机进行二次验证（check_task）
        5. 如果验证失败，执行 recovery 恢复流程
        6. 返回起点
        7. 调用 request_exit 结束任务
        
        注意:
        - 使用父类的 _goto_and_face_target, check_task, run_recovery 方法
        - 这些方法继承自 TaskSubscriber，与 query_task_2_3 保持一致
        """
        with self._lock:
        ...
        self.get_logger().info(f"导航到目标: ({target_x:.2f}, {target_y:.2f})")
