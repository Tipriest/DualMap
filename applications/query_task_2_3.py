"""
Task 2-3 V5: 指令遵从 - 找到目标物体（带相关物体定位）+ 提前0.5m截断

新逻辑：
1. 有房间 + 有related：在房间内找related → 成功则搜索target，失败则fallback到锚点
2. 有房间 + 无related：直接去房间锚点 → fallback搜索target
3. 无房间 + 有related：提示使用query++（不支持）
4. 无房间 + 无related：提示使用query++（不支持）

V5新增功能：
- 导航到目标时提前0.5m截断，避免撞到目标物体
- 使用 _goto_point_with_early_stop 方法
...
class Task23Subscriber(TaskSubscriber):
    ...
    def __init__(self, cfg_path: str):
        ...
        self.max_spin_rounds = 5  # 最多旋转 3 圈
        self.current_goal_handle = None  # 用于提前截断导航
        # 停止父类的 worker
    ...
    def _goto_point_with_early_stop(
        ...
        def _on_goal_response(fut):
            try:
                ...
                self.current_goal_handle = gh  # 保存用于取消
...
def parse_command_with_qwen(cfg_path: str, user_query: str, memory_context: str = ""):
    """
    使用 Qwen API 解析用户指令，支持记忆上下文推理

    Args:
        cfg_path: 配置文件路径
        user_query: 任务指令
        memory_context: 记忆上下文（可选，用于高阶任务）
...
* 玩具/毛绒玩具/玩偶→soft toy, 地毯→carpet, 台灯→table lamp
* 床头柜→nightstand, 帐篷→tent
* 书架→bookshelf, 燃气灶/煤气灶→gas stove, 锅→pot, 水壶/烧水壶→kettle
* 菜篮/食物篮→food basket, 水龙头→faucet, 被子→quilt
...
- 地面/地板（floor/ground/carpet）不算 related_object
...
- 物体名称必须从以下列表中选择对应的英文单词：
  * 书包-backpack
  * 相框→picture frame, 篮球→basketball, 碗→bowl
  * 香蕉→banana, 苹果→apple, 木马/玩具马→toy horse, 椅子→chair
  * 沙发→couch, 绿植/植物→green plant, 床→bed, 桌子/书桌→table
  * 电视→tv, 笔记本电脑→laptop, 微波炉→microwave, 柜子→cabinet
  * 玩具/毛绒玩具/玩偶→soft toy, 地毯→carpet, 台灯→table lamp
...
"找木马" → {{"target_room": "None", "target_object": "toy horse", "related_object": "None"}}
"桌子上的篮球" → {{"target_room": "None", "target_object": "basketball", "related_object": "table"}}
...
def main():
    ...
    print("=" * 60)
    print("Task 2-3: 指令遵从 - 找到目标物体（支持记忆推理）")
    print("=" * 60)
    ...
    # 询问是否使用记忆上下文（高阶任务）
    print("\n请选择任务模式：")
    print("  1 - 普通任务（直接输入指令，如'去卧室拿床上的被子'）")
    ...
    write_log(f"开始任务: 指令='{query_text}'" + (f", 记忆='{memory_context[:50]}...'" if memory_context else ""))
