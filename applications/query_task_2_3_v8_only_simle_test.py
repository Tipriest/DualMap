...
class Task23Subscriber(TaskSubscriber):
    ...
    def _task_worker_v2(self):
        ...
        self.target_y =-0.84
        if target_found:
            self._navigate_and_check_target("chair")
        else:
    ...
    def _navigate_and_check_target(self, target_name: str):
        ...
        if not nav_ok:
            ...
            for i in range(1, 10):
                self.get_logger().warn(f"⚠️  导航到计算点失败，尝试直接走向目标， 第{10-i}/10个点")
                new_point_x = self.target_x - i /10.0 * dx
                new_point_y = self.target_y - i /10.0 * dy
                nav_ok = self._goto_point_with_early_stop(
...
def main():
    ...
    # node._navigate_and_check_target("chair")
    # 启动执行器
