...
class Task23Subscriber(TaskSubscriber):
    ...
    def _navigate_and_check_target(self, target_name: str):
        ...
        if not nav_ok:
            self.get_logger().warn("⚠️  导航到计算点失败，尝试直接走向目标")
            # 回退策略：面向目标，直接走到尽可能近的位置
            current_x, current_y = self.current_x, self.current_y
            dx = target_x - current_x
            dy = target_y - current_y
            fallback_yaw = math.atan2(dy, dx)
            
            self.get_logger().info(f"→ 面向目标 ({target_x:.2f}, {target_y:.2f})，走到最近位置")
            fallback_ok = self._goto_point_with_early_stop(
                target_x, target_y, fallback_yaw,
                frame_id="map", wait_timeout=5.0, stop_distance=0.5
            )
            
            if not fallback_ok:
                self.get_logger().error("❌ 回退策略也失败")
                self._return_home_and_exit("Navigation failed after fallback")
                return
