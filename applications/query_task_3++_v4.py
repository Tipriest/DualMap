...
- 物品名称需要是英文的类型，如"杯子"返回"cup"，"桌子"返回"table"
...
class Task3PPSubscriber(TaskSubscriber):
    ...
    def _handle_target_found(self):
        ...
        self.get_logger().info(f"→ 导航到'{self.target_name}'...")
        ...
        # 导航并面向目标
        nav_ok = self._goto_and_face_target(free_x, free_y, target_x, target_y)
    ...
    def _handle_all_rooms_not_found(self):
        ...
        self._goto_point(0.0, -0.3, yaw=0.0, frame_id="map", wait_timeout=5.0)
