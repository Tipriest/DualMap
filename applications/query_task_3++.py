...
def parse_command_with_qwen(cfg_path: str, user_query: str, memory_context: str = ""):
    """
    使用 Qwen API 解析用户指令或从记忆上下文推理

    Args:
        cfg_path: 配置文件路径
        user_query: 任务指令
        memory_context: 可选的记忆上下文（用于高阶任务推理）
...
* 书架→bookshelf, 燃气灶/煤气灶→gas stove, 锅→pot, 水壶/烧水壶→table
* 菜篮/食物篮→food basket, 水龙头→faucet

【推理任务】
1. **target_object**: 从记忆上下文中找到任务指令要找的物品（必须存在）
2. **target_room**: ⚠️ 必须始终返回 "None"（因为物品位置会变化，不能固定到具体房间）
3. **related_object**: 从记忆上下文推理出辅助定位的相关物体（如"煤气灶旁的水壶"→related是"gas stove"）
4. **room_priority**: ⚠️ 这是核心输出！根据记忆上下文推理该物品最可能在哪些房间，按可能性从高到低排序所有4个房间

【推理规则】
- 仔细阅读记忆上下文，找到任务指令中提到的物品及其位置描述
- target_object 必须从词汇表中选择准确的英文名称
- target_room 必须始终为 "None"（即使记忆中提到了房间，也要返回 None，因为物品会移动）
- related_object 是帮助定位的参照物（如"桌上的书包"→related是"table"），如果没有则为 "None"
- 地面/地板（floor/ground）不应作为 related_object
- room_priority 是最重要的输出：
  * 根据记忆上下文推理物品最可能在哪个房间，将其排在第一位
  * 例如：记忆提到"妈妈在厨房用过水壶" → room_priority应该是 ["kitchen", ...]
  * 必须包含所有4个房间，只是调整优先级顺序
...
指令：请找到妈妈早上在厨房用过的烧水壶
输出：（注意：即使记忆提到厨房，target_room也必须是None，但kitchen排第一）
{{
    "target_object": "table",
    "target_room": "None",
...
"""
        ...
        # 普通任务：直接提取模式
        prompt = f"""
你是一个智能机器人助手，需要根据用户指令理解任务并推理搜索策略。
...
【理解任务】
1. **target_object**: 从指令中识别需要找到的物品（必须存在）
2. **target_room**: ⚠️ 必须始终返回 "None"（因为物品位置会变化，不能固定到具体房间）
3. **related_object**: 从指令中识别辅助定位的相关物体（如"床上的被子"→related是"bed"，"桌上的篮球"→related是"table"），如果没有则为 "None"
4. **room_priority**: ⚠️ 这是核心输出！根据指令和常识推理房间优先级，按可能性从高到低排序所有4个房间

【推理规则】
- target_object 必须从词汇表中选择准确的英文名称
- target_room 必须始终为 "None"（即使指令中提到了房间如"去厨房找水壶"，也要返回 None）
- 注意：如果指令中提到房间（如"去厨房"），不要提取到 target_room，而是体现在 room_priority 中
- related_object 是帮助定位的参照物（如"桌上的书包"→related是"table"），如果没有则为 "None"
- 地面/地板（floor/ground）不应作为 related_object
- room_priority 是最重要的输出，必须包含所有4个房间，根据以下规则推理优先级：
  * 如果指令明确提到房间（如"去厨房找水壶"），将该房间排在第一位：["kitchen", ...]
  * 否则根据物品类型推理：
    - 水壶、锅、煤气灶、菜篮 → kitchen最优先
    - 床、床头柜、台灯、被子 → bedroom最优先
    - 玩具马、帐篷、积木、毛绒玩具 → childroom最优先
    - 沙发、电视、地毯 → livingroom最优先
    - 书桌、笔记本电脑、书包 → bedroom或childroom优先
    - 如果物品类型不明确，livingroom作为首选
...
【示例】
指令："床上的木马"
输出：{{"target_object": "toy horse", "target_room": "None", "related_object": "bed", "room_priority": ["childroom", "bedroom", "livingroom", "kitchen"]}}

指令："找篮球"
输出：{{"target_object": "basketball", "target_room": "None", "related_object": "None", "room_priority": ["livingroom", "childroom", "bedroom", "kitchen"]}}

指令："去厨房找水壶"（注意：即使提到厨房，target_room也是None，但kitchen排第一）
输出：{{"target_object": "table", "target_room": "None", "related_object": "None", "room_priority": ["kitchen", "livingroom", "bedroom", "childroom"]}}

指令："请找到妈妈早上在厨房用过的烧水壶"（注意：target_room是None，但kitchen排第一）
输出：{{"target_object": "table", "target_room": "None", "related_object": "None", "room_priority": ["kitchen", "livingroom", "bedroom", "childroom"]}}

指令："去儿童房找帐篷旁边的积木"（注意：target_room是None，但childroom排第一）
输出：{{"target_object": "building blocks", "target_room": "None", "related_object": "tent", "room_priority": ["childroom", "bedroom", "livingroom", "kitchen"]}}
...
class Task3PPSubscriber(TaskSubscriber):
    ...
    def __init__(self, cfg_path: str):
        ...
        # ===== 1) 停掉父类 worker（关键：self._worker = None 不会停线程）=====
        base_worker = getattr(self, "_worker", None)

        # 通知父线程退出（父线程 while not self._shutdown_event.is_set() ...）
        self._shutdown_event.set()

        # 等父线程收尾一下，避免它抢到_task_event后抛错
        if base_worker is not None:
            base_worker.join(timeout=1.0)

        # 父线程已退出后，再清掉 shutdown 标志，让我们自己的 worker 能继续工作
        self._shutdown_event.clear()

        # ===== 2) 对 3++：彻底不需要 room_ready gating（防御性）=====
        self.require_room_filter = False   # 防止任何残留路径走父类“等房间”逻辑
        self.is_room_ready = True          # 可选：保险起见
        self.room_bbox = None              # 可选
    ...
    def _task_worker_3pp(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                # ========== 场景判断 ==========
                # 注意：忽略 room 字段（即使LLM错误返回了房间也不管）
                # 始终使用 room_priority 进行全屋遍历
                has_related = related_name and related_name != "None"

                # 场景1: 有related（全屋遍历，每个房间找related+target）
                if has_related:
                    self.get_logger().info("=" * 60)
                    self.get_logger().info("📍 场景1: 有related物体")
                    self.get_logger().info(f"  策略: 按优先级遍历所有房间 → 找related → 验证包含关系 → 找target")
                    self.get_logger().info(f"  房间顺序: {' → '.join(self.room_priority) if self.room_priority else '默认'}")
                    self.get_logger().info("=" * 60)
                    self._handle_no_room_with_related(target_name, related_name)
                    continue

                # 场景2: 无related（全屋遍历，直接找target）
                self.get_logger().info("=" * 60)
                self.get_logger().info("📍 场景2: 无related物体")
                self.get_logger().info(f"  策略: 按优先级遍历所有房间 → 直接找target → 转圈搜索")
                self.get_logger().info(f"  房间顺序: {' → '.join(self.room_priority) if self.room_priority else '默认'}")
                self.get_logger().info("=" * 60)
    ...
    def _return_home_and_exit(self, reason: str):
        ...
        write_log("返回起点 (-0.7,0.7)")

        return_ok = self._goto_point(-0.7, 0.7, yaw=0.0, frame_id="map", wait_timeout=5.0)
    ...
    def _spin_three_rounds(self):
        ...
        原地旋转3圈（兼容原有接口）
        """
        self._spin_and_wait_for_target(3)
    ...
    def _handle_all_rooms_not_found(self):
        ...
        # 返回原点(y=-0.3)
        self.get_logger().info("返回原点 (-0.7, 0.7)")
...
def main():
    ...
    print("\n请选择任务模式：")
    print("  1 - 普通任务（直接输入指令）")
    print("  2 - 高阶任务（使用记忆上下文推理）")
    mode_choice = input("请输入模式编号 (1 or 2, 默认1): ").strip()

    memory_context = ""
    if mode_choice == "2":
        # 高阶任务模式：需要记忆上下文
        print("\n【记忆上下文】请输入记忆描述：")
        print("爸爸日常会在书桌上工作，他的黑色书包放在书桌中央；卧室床头柜的台灯是妈妈睡前阅读用的；孩子的书包是橙色的，放学后经常在儿童房里玩玩具，书包一般放在房间的书架上；妈妈经常在厨房准备美食，她早晨用过煤气灶旁的水壶烧开水；卧室的毛绒玩具狗是孩子睡前抱着睡的，所以总放在床头。")
        memory_context = input("记忆上下文: ").strip()

        if not memory_context:
            print("[ERROR] 高阶任务模式下记忆上下文不能为空！")
            node.destroy_node()
            rclpy.shutdown()
            return
    ...
    write_log(f"开始任务: 模式={'高阶' if memory_context else '普通'}, 指令='{query_text}'")

    # LLM 解析（根据模式传入不同参数）
    qwen_result = parse_command_with_qwen(cfg_path, query_text, memory_context)
