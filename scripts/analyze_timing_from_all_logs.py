...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze frame timing from DualMap logs.\n"
            "- If --log-file is given: analyze that single file.\n"
            "- Otherwise: analyze all *.log files under --log-dir."
        )
    )
    parser.add_argument(
        "--log-file",
        type=str,
        default=None,
        help="Path to a specific log file. If set, only this file is analyzed.",
    )
    parser.add_argument(
        "--log-dir",
        type=str,
        default=None,
        help=(
            "Directory containing log_*.log files "
            "(default: output/map_results/log under project root)."
        ),
    )
    parser.add_argument(
        "--show",
    ...
    if args.log_file:
        ...
        if log_path.is_file():
            logs_to_process = [log_path]
        else:
            print(f"Specified log file not found: {log_path}")
            return
    else:
        if not log_dir.exists():
            print(f"Log directory not found: {log_dir}")
            return
        logs_to_process = sorted(log_dir.glob("*.log"))
        if not logs_to_process:
            print(f"No .log files found under {log_dir}")
            return

    print(
        f"Found {len(logs_to_process)} log file(s) to analyze under {log_dir}"
    )
    ...
    for log_path in logs_to_process:
        ...
        if not frame_indices:
            print(f"  No timing information found in {log_path.name}, skipped.")
            continue
